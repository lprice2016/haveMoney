...
> 当前产品方向见 `docs/11-product-decision-ledger.md`。本文件记录 GStack + 三省六部的协作架构；业务职责已从早期“虚拟财富系统”调整为“私人管家式有钱爽剧”。
...
| 户部 | `hm-growth-loop` / `hm-delivery-gate` | 成本、收费、指标、分享闭环、是否允许进入实现 | “这个玩法会不会亏钱，能不能进入 H5 原型” |
| 礼部 | `hm-design-language` / `hm-content-lab` | 管家语气、奢华细节、短剧分镜、分享战报、品牌调性 | “把这个愿望写成一段有钱爽剧” |
...
用于生成、比较、评分和改写愿望、爽剧分镜、富豪战报和分享文案。

### `hm-design-language`

用于定义管家式语气、奢华细节、仪式感、分享卡和视觉表达。重点是可信细节、剧情递进和不让老板做小决定。
...
用于设计分享卡、朋友点击后的接梗路径、首批用户冷启动和传播验证。

### `hm-delivery-gate`

用于决定继续人工内容测试、进入 H5 + 薄服务端原型，还是暂缓实现。
...
| 反馈爽点 | 被识别、被优先、被预判、被见证、改规则、战报是什么？ |
| 风险边界 | 是否涉及真实金融、真实消费诱导或身份压力？ |
| MVP 验收 | 最小可验证结果是什么？ |

## 下一步

1. 用 `hm-product-gate` 判断每个新想法是否值得进入验证。
2. 用 `hm-content-lab` 和 `hm-design-language` 继续打磨真人测试里的爽剧样例。
3. 用 `hm-risk-review` 审查色情、赌博、金融误导和低俗炫富边界。
4. 用 `hm-growth-loop` 验证今日富豪战报是否能被截图、发群和接梗。
5. 达到建设门禁后，再用 `hm-delivery-gate` 决定是否进入 H5 + 薄服务端 + AI 代理原型。
